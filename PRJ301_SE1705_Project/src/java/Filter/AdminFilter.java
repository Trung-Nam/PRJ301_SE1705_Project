...
public class AdminFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session.getAttribute("admin") == null && !(url.endsWith("admin/login-admin"))) {
            session.setAttribute("mess", "Bạn không có quyền truy cập vào trang này khi chưa đăng nhập.");
            res.sendRedirect("login-admin");
        } else if (((session.getAttribute("admin") == null) || (session.getAttribute("admin") != null))
                && (url.endsWith(".jsp") || url.endsWith(".html"))) {
            req.getRequestDispatcher("page404.jsp").forward(req, res);
        } else if ((session.getAttribute("user") != null)) {
            String serverName = req.getServerName();
            String path = req.getContextPath();
            int portServer = req.getLocalPort();
            System.out.println(" Init method is called in " + serverName);
            System.out.println(" Init method is called in " + path);
            System.out.println(" Init method is called in " + portServer);
            req.setAttribute("link", path + "/Home");
            req.getRequestDispatcher("page403.jsp").forward(req, res);
        } else {
